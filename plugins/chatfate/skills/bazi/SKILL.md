---
name: bazi
description: 根据 ChatFate 八字排盘与用户自述，撰写今日指引、年度主题或人生命盘的自然中文报告。仅在总编排已取得 chatfate.bazi.chart.v2 后使用；不手工排盘，不替用户编造经历。
---

# 八字解读

如果工具已返回 `report_ready`，直接由总编排交付原报告，不重新生成解释。其余新报告统一输出 `chatfate.reading.interpretation.v1`；旧版报告继续由原合同读取，不要求迁移。

## 先看所问与现有依据

读取本次 `calculatedFacts`、`reading` 和可选 `userContext`。八字工具会隐藏出生输入，`normalizedInput` 缺席不影响解读。不要为补充故事而追问私密经历，也不把卡片外的猜测写成用户自述。

- life 从日主、时令、显干藏干与柱间关系形成主线，再选择真正相关的生活主题；不强制凑齐事业、财富、感情、健康四栏。
...
- `product`: 本次的 `daily`、`annual` 或 `life`
...
标题不必固定，篇幅由信息决定。今日通常两章就够；年度与人生需要更充分的依据时再展开。teaser 不制造悬念，不以凶煞、隐患或付费揭晓引诱解锁；付费章不能只有免责声明、术语表或通用的“确认—安排—复盘”。
...
- 年月柱按实际交节时刻；真太阳时仅影响日时柱。日期型资料取北京时间正午，交节日不代表全天不变。大运表只列年份时，不声称知道精确交运月日。
